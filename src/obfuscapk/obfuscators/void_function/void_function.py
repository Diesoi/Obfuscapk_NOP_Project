...
class VoidFunction(obfuscator_category.ICodeObfuscator):
    ...
    def add_empty_function(self, smali_file):
        ...
        random.seed()
        nums = [random.randint(1, 7) for _ in range(2)]
        function_definition = (
            ...
            "\treturn-void\n"
            ".end method\n"
        ).format(function_name, hex(nums[0]), hex(nums[1]))
    ...
    def add_call_to_void(self, smali_file):
        self.logger.debug(
            'Inserting call to void add function in file "{0}"'.format(smali_file))
    ...
    def treat_dex(self, smali_files, max_methods_to_add, interactive):
        ...
        for smali_file in util.show_list_progress(
                ...
                description="Inserting call to void add function in smali files"):
            if added_methods < max_methods_to_add:
                self.add_call_to_void(smali_file)
                added_methods += 1
            else:
                break
